...
const defaultLinks = links.filter((link) => link.allow.some((role) => role !== "admin_alfa"));
export function AdminSidebar() {
  ...
  useEffect(() => {
    ...
    async function loadRoles() {
      ...
      const userRoles = (data as UserRoleRow[] | null)
        ?.flatMap((row) => {
          if (!row.role) return [];
          if (Array.isArray(row.role)) return row.role.map((r) => r.name);
  ...
  const visibleLinks = useMemo(() => {
    if (loadingRoles) return defaultLinks;
    if (!roles.length) return defaultLinks;
