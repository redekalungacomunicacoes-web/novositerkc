...
import { buildPeriodsBetween, computeDashboardData, computeFundSummaries, computeProjectSummaries } from '../core/financeEngine';
...
export function useFinanceSupabase() {
  ...
  const getDashboardAggregates = useCallback(
    async (fundsList: FinanceiroFundo[], projectsList: FinanceiroProjeto[], paidAllMovements: FinanceiroMovimentacao[]): Promise<DashboardData> => {
      ...
      const { data: movementsRangeData, error: movementsRangeError } = await buildMovementsQuery({ startDate: start, endDate: end });
      ensure(movementsRangeError, 'Falha ao carregar dados do dashboard.');

      const allInRangeRaw = (movementsRangeData || []).map((row) => mapMovement(row as AnyRow));

      // Normaliza fundoId usando projects já carregados (evita depender de joins do Supabase)
      const projectFundById = new Map(projectsList.map((p) => [p.id, p.fundoId]));
      const fundsNameById = new Map(fundsList.map((f) => [f.id, f.nome]));
      const allInRange = allInRangeRaw.map((m) => {
      ...
      const paidInRange = allInRange.filter((m) => m.status === 'pago');
      const paidAll = paidAllMovements;

      const periods = buildPeriodsBetween(start, end);

      return computeDashboardData({
        funds: fundsList,
        paidMovementsInRange: paidInRange,
        paidMovementsAll: paidAll,
        allMovementsInRange: allInRange,
        periods,
      });
    },
    [],
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const fundsWithCalculatedBalance = computeFundSummaries(fundsList, paidFundsMovements);
      const projectsWithCalculatedBalance = computeProjectSummaries(projectsList, paidFundsMovements);

      setFunds(fundsWithCalculatedBalance);
      setProjects(projectsWithCalculatedBalance);
      ...
      const [latest, categoriesList, aggregates] = await Promise.all([
        ...
        listCategories(),
        getDashboardAggregates(fundsList, projectsList, paidFundsMovements),
      ]);
