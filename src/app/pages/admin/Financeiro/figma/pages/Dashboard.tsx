...
export function Dashboard() {
  ...
  const load = async () => {
    const [fundsData, projectData, latestRows, aggregate] = await Promise.all([listFunds(), listProjects(), listLatestMovements(10), getDashboardAggregates({ months: 6 })]);
    setFunds(fundsData || []);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KPICard title="Saldo Atual" value={formatCurrency(Number(dashboardData.kpis.currentBalance || 0))} subtitle="Entradas - Saídas (pagas)" icon={Wallet} color="primary" />
        <KPICard title="Entradas (pagas)" value={formatCurrency(Number(dashboardData.kpis.totalIn || 0))} subtitle="Últimos 6 meses" icon={TrendingUp} color="success" />
        <KPICard title="Saídas (pagas)" value={formatCurrency(Number(dashboardData.kpis.totalOut || 0))} subtitle="Últimos 6 meses" icon={TrendingDown} color="danger" />
