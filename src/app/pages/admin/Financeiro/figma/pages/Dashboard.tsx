...
export function Dashboard() {
  ...
  const load = async () => {
    try {
      ...
      const counts = rows.reduce<Record<string, number>>(
        (acc, row) => ({
          ...acc,
          [row.movement_id]: (acc[row.movement_id] || 0) + 1,
        }),
        {}
      );
      ...
      try {
        ...
        setDashboardData({
          kpis: {
            ...
            totalIn: 0,
            totalOut: 0,
            currentBalance: (fundsData || []).reduce(
      ...
      if (import.meta.env.DEV) console.error(error);
      setFeedback(
        (error as Error)?.message || "Erro ao carregar dashboard financeiro."
      );
    }
  };
  ...
  const totalCash = funds.reduce(
    (acc, fund) => acc + Number(fund.current_balance || 0),
    0
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KPICard
          ...
          value={formatCurrency(
            Number(totalCash || dashboardData.kpis.currentBalance || 0)
          )}
              ...
              {latestMovements.map((mov) => (
                <tr key={mov.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">
                    {projectMap.get(mov.project_id) || "-"} /{" "}
                    {fundMap.get(mov.fund_id) || "-"}
                  </td>
