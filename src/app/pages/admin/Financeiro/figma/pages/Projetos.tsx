...
export function Projetos() {
  const { listProjects, listFunds } = useFinanceSupabase();
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <Link to="/admin/financeiro/novo" className="flex items-center gap-2 px-6 py-3 bg-[#0f3d2e] text-white rounded-xl hover:bg-[#0a2b20] transition-colors shadow-sm">
          ...
          return (
            <div key={projeto.id} className="bg-white rounded-xl border-2 border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden">
              ...
              <div className="p-6">
                ...
                <Link to={`/admin/financeiro/projetos/${projeto.id}`} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#0f3d2e] text-white rounded-lg hover:bg-[#0a2b20] transition-colors"><Eye className="w-5 h-5" />Ver Detalhes</Link>
              </div>
            </div>
          );
        })}
