...
export function FundoDetalhes() {
  ...
  const metrics = useMemo(() => {
    ...
    const entradasPagas = movimentacoesFundo.filter((m) => m.tipo === 'entrada' && m.status === 'pago').reduce((acc, m) => acc + m.valorTotal, 0);
    const saidasPagas = movimentacoesFundo.filter((m) => m.tipo === 'saida' && m.status === 'pago').reduce((acc, m) => acc + m.valorTotal, 0);

    return {
      saldoDisponivel: fundo?.saldoAtual || 0,
      orcado,
      gastoReal: gastoPago,
      execucao: orcado > 0 ? (gastoPago / orcado) * 100 : 0,
      comprovantesPct: movimentacoesFundo.length > 0 ? (withAttachments / movimentacoesFundo.length) * 100 : 0,
      pendencias: movimentacoesFundo.filter((m) => m.status === 'pendente').length,
      semDocumento: movimentacoesFundo.length - withAttachments,
      saldoInicial: fundo?.saldoInicial || 0,
      entradasPagas,
      saidasPagas,
      saldoCaixa: (fundo?.saldoInicial || 0) + entradasPagas - saidasPagas,
    };
  ...
  return (
    <div className="space-y-5 p-8">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/admin/financeiro/fundos" className="inline-flex items-center gap-2 text-sm text-[#0f3d2e] hover:underline"><ArrowLeft className="h-4 w-4" />Voltar para fundos</Link>
          <h1 className="mt-2 text-2xl font-semibold">{fundo.nome}</h1>
        </div>
      ...
      {tab === 'prestacao' && (
        <div className="space-y-3 rounded-xl border bg-white p-6">
          ...
          {movimentacoesFundo.filter((mov) => mov.status === 'pendente' || (mov.attachmentsCount || mov.comprovantes.length) === 0).map((mov) => (
      ...
      <ModalMovimentacao isOpen={openModal} onClose={() => setOpenModal(false)} editData={editingMovement} projects={projects.map((p) => ({ id: p.id, name: p.nome, fundId: p.fundoId }))} funds={funds.map((f) => ({ id: f.id, name: f.nome }))} categories={categories} attachments={attachments} onSubmit={(payload: MovementPayload) => editingMovement ? updateMovement(editingMovement.id, payload) : createMovement({ ...payload, fund_id: id, project_id: payload.project_id || '' })} onDelete={editingMovement ? async () => { await deleteMovementCascade(editingMovement.id); setOpenModal(false); } : undefined} onUploadAttachment={uploadAttachment} onDeleteAttachment={deleteAttachment} onViewAttachment={viewAttachment} onChanged={refreshAll} />
