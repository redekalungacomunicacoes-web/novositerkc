...
export function FundoDetalhes() {
  ...
  useEffect(() => {
    void refreshAll({ withAttachmentsFor: editingMovement?.id ?? null });
  ...
  const metrics = useMemo(() => {
    const orcado = Number(fundo?.totalOrcado || 0);

    const gastoPago = movimentacoesFundo
    ...
    const entradasPagas = movimentacoesFundo
      .filter((m) => m.tipo === 'entrada' && m.status === 'pago')
      .reduce((acc, m) => acc + m.valorTotal, 0);

    const saidasPagas = movimentacoesFundo
      .filter((m) => m.tipo === 'saida' && m.status === 'pago')
      .reduce((acc, m) => acc + m.valorTotal, 0);

    const saldoInicial = Number(fundo?.saldoInicial || 0);

    return {
      saldoDisponivel: Number(fundo?.saldoAtual || 0),
      orcado,
      gastoReal: gastoPago,
      ...
      entradasPagas,
      saidasPagas,
      saldoCaixa: saldoInicial + entradasPagas - saidasPagas,
    };
  ...
  return (
    <div className="space-y-5 p-8">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/admin/financeiro/fundos" className="inline-flex items-center gap-2 text-sm text-[#0f3d2e] hover:underline">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded bg-[#0f3d2e] px-4 py-2 text-sm text-white"
        >
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="rounded-xl border bg-white p-4">
              <h3 className="mb-3 text-sm font-semibold">Fluxo mensal</h3>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={fluxoMensal}>
                  ...
                  <Legend />
                  <Line dataKey="entradas" stroke="#10b981" />
            ...
            <div className="rounded-xl border bg-white p-4">
              <h3 className="mb-3 text-sm font-semibold">Orçado vs real por categoria</h3>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={categoriaRows}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="categoria" />
                  <YAxis />
                  <Tooltip formatter={(v: number) => formatCurrency(v)} />
                  <Legend />
                  <Bar dataKey="orcado" fill="#ffdd9a" />
                  <Bar dataKey="real" fill="#0f3d2e" />
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="rounded-xl border bg-white p-4">
              <h3 className="mb-3 text-sm font-semibold">Distribuição categorias</h3>
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie data={categoriaRows} dataKey="real" nameKey="categoria" outerRadius={80} fill="#0f3d2e" />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="border-b bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs uppercase">Projeto</th>
                    <th className="px-4 py-3 text-left text-xs uppercase">Valor</th>
                    <th className="px-4 py-3 text-left text-xs uppercase">Comprovantes</th>
                ...
                <tbody>
                  {movimentacoesFundo.map((mov) => (
                    <tr key={mov.id} className="border-b">
                      <td className="px-4 py-3 text-sm">{formatDate(mov.data)}</td>
                      <td className="px-4 py-3 text-sm">{mov.titulo}</td>
                      <td className="px-4 py-3 text-sm">{projects.find((p) => p.id === mov.projetoId)?.nome ?? '—'}</td>
                      <td className="px-4 py-3 text-sm">{formatCurrency(mov.valorTotal)}</td>
                      <td className="px-4 py-3 text-sm">{mov.attachmentsCount || mov.comprovantes.length}</td>
                      <td className="px-4 py-3">
                        <div className="flex justify-center gap-2">
                          <button
                            onClick={async () => {
                              setEditingMovement(mov);
                              setAttachments(await listAttachments(mov.id));
                              setOpenModal(true);
                            }}
                          >
              ...
              {movimentacoesFundo.length === 0 && <p className="p-4 text-sm text-gray-500">Sem movimentações.</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'relatorios' && (
        <div className="flex flex-wrap gap-3 rounded-xl border bg-white p-6">
          <button
            className="rounded border px-4 py-2 text-sm"
            onClick={() =>
              downloadCsv(
                'relatorio-completo-fundo.csv',
                movimentacoesFundo.map((mov) => ({
                  data: mov.data,
                  titulo: mov.titulo,
                  projeto_id: mov.projetoId,
                  categoria: mov.categoria,
                  valor_total: mov.valorTotal,
                  status: mov.status,
                })),
              )
            }
          ...
          <button className="inline-flex items-center gap-2 rounded border px-4 py-2 text-sm">
      ...
      <ModalMovimentacao
        ...
        onSubmit={(payload: MovementPayload) =>
          editingMovement
            ...
            : createMovement({ ...payload, fund_id: id, project_id: payload.project_id || '' })
        }
        onDelete={
          editingMovement
            ? async () => {
        ...
        onChanged={() => refreshAll({ withAttachmentsFor: editingMovement?.id ?? null })}
      />
