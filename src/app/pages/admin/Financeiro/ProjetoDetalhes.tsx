...
export function ProjetoDetalhes() {
  ...
  return (
    <div className="space-y-5 p-8">
      ...
      <ModalMovimentacao isOpen={openModal} onClose={() => setOpenModal(false)} editData={editingMovement} projects={projects.map((p) => ({ id: p.id, name: p.nome }))} funds={funds.map((f) => ({ id: f.id, name: f.nome }))} categories={categories} attachments={attachments} onSubmit={(payload: MovementPayload) => editingMovement ? updateMovement(editingMovement.id, payload) : createMovement({ ...payload, project_id: id, fund_id: payload.fund_id || projeto.fundoId })} onDelete={editingMovement ? async () => { await deleteMovementCascade(editingMovement.id); setOpenModal(false); } : undefined} onUploadAttachment={async (file, movementId, payload) => uploadAttachment(file, movementId, payload.fund_id || projeto.fundoId, payload.project_id || id)} onDeleteAttachment={deleteAttachment} onChanged={refreshAll} />
