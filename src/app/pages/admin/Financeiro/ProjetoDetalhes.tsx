...
export function ProjetoDetalhes() {
  ...
  const [movimentacoesProjeto, setMovimentacoesProjeto] = useState<FinanceiroMovimentacao[]>([]);
  ...
  const refreshAll = async () => {
    await load();
    const rows = await listMovements({ projectId: id });
  ...
  const metrics = useMemo(() => {
    ...
    const entradasPagas = movimentacoesProjeto.filter((m) => m.tipo === 'entrada' && m.status === 'pago').reduce((acc, m) => acc + m.valorTotal, 0);
    const saidasPagas = movimentacoesProjeto.filter((m) => m.tipo === 'saida' && m.status === 'pago').reduce((acc, m) => acc + m.valorTotal, 0);
    ...
    return {
      ...
      saldoInicial: projeto?.totalOrcado || 0,
      entradasPagas,
      saidasPagas,
      saldoCaixa: (projeto?.totalOrcado || 0) + entradasPagas - saidasPagas,
    };
  ...
  const fluxoMensal = useMemo(() => {
    ...
    movimentacoesProjeto.forEach((mov) => {
      ...
      if (mov.tipo === 'entrada' && mov.status === 'pago') current.entradas += mov.valorTotal;
      if (mov.tipo === 'saida' && mov.status === 'pago') current.saidas += mov.valorTotal;
  ...
  const categoriaRows = useMemo(() => {
    ...
    movimentacoesProjeto
      ...
      .forEach((m) => map.set(m.categoria || 'Sem categoria', (map.get(m.categoria || 'Sem categoria') ?? 0) + m.valorTotal));
  ...
  return (
    <div className="space-y-5 p-8">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/admin/financeiro/projetos" className="inline-flex items-center gap-2 text-sm text-[#0f3d2e] hover:underline"><ArrowLeft className="h-4 w-4" />Voltar para projetos</Link>
          <h1 className="mt-2 text-2xl font-semibold">{projeto.nome}</h1>
        </div>
        <button onClick={() => { setEditingMovement(null); setAttachments([]); setOpenModal(true); }} className="inline-flex items-center gap-2 rounded bg-[#0f3d2e] px-4 py-2 text-sm text-white"><Plus className="h-4 w-4" />Nova Movimentação</button>
      </div>

      <div className="flex gap-2">
        {(['geral', 'caixa', 'prestacao', 'relatorios'] as const).map((item) => (
          <button key={item} className={`rounded px-3 py-2 text-sm ${tab === item ? 'bg-[#0f3d2e] text-white' : 'border'}`} onClick={() => setTab(item)}>
      ...
      {tab === 'geral' && (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border bg-white p-4"><h3 className="mb-3 text-sm font-semibold">Distribuição categorias</h3><ResponsiveContainer width="100%" height={240}><PieChart><Pie data={categoriaRows} dataKey="real" nameKey="categoria" outerRadius={80} fill="#0f3d2e" /></PieChart></ResponsiveContainer></div>
          </div>
        </>
      )}
      ...
      {tab === 'caixa' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <div className="rounded-xl border bg-white p-4"><p className="text-xs">Saldo inicial</p><p className="text-lg font-semibold">{formatCurrency(metrics.saldoInicial)}</p></div>
          ...
          <div className="overflow-hidden rounded-xl border bg-white"><div className="overflow-x-auto"><table className="w-full"><thead className="border-b bg-gray-50"><tr><th className="px-4 py-3 text-left text-xs uppercase">Data</th><th className="px-4 py-3 text-left text-xs uppercase">Título</th><th className="px-4 py-3 text-left text-xs uppercase">Fundo</th><th className="px-4 py-3 text-left text-xs uppercase">Valor</th><th className="px-4 py-3 text-left text-xs uppercase">Comprovantes</th><th className="px-4 py-3 text-center text-xs uppercase">Ações</th></tr></thead><tbody>{movimentacoesProjeto.map((mov) => <tr key={mov.id} className="border-b"><td className="px-4 py-3 text-sm">{formatDate(mov.data)}</td><td className="px-4 py-3 text-sm">{mov.titulo}</td><td className="px-4 py-3 text-sm">{funds.find((f) => f.id === mov.fundoId)?.nome ?? '—'}</td><td className="px-4 py-3 text-sm">{formatCurrency(mov.valorTotal)}</td><td className="px-4 py-3 text-sm">{mov.attachmentsCount || mov.comprovantes.length}</td><td className="px-4 py-3"><div className="flex justify-center gap-2"><button onClick={async () => { setEditingMovement(mov); setAttachments(await listAttachments(mov.id)); setOpenModal(true); }}><Edit className="h-4 w-4" /></button><button onClick={() => { setSelectedMovement(mov); setOpenDelete(true); }}><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table>{movimentacoesProjeto.length === 0 && <p className="p-4 text-sm text-gray-500">Sem movimentações.</p>}</div></div>
        </div>
      )}

      {tab === 'prestacao' && (
        <div className="space-y-3 rounded-xl border bg-white p-6">
          <p className="text-sm text-gray-600">Pendentes: {metrics.pendencias}</p>
          <p className="text-sm text-gray-600">Sem documento: {metrics.semDocumento}</p>
          {movimentacoesProjeto.filter((mov) => mov.status === 'pendente' || (mov.attachmentsCount || mov.comprovantes.length) === 0).map((mov) => (
            <div className="flex items-center justify-between rounded border p-3 text-sm" key={mov.id}>
              ...
              <button className="inline-flex items-center gap-1 rounded border px-2 py-1" onClick={async () => { setEditingMovement(mov); setAttachments(await listAttachments(mov.id)); setOpenModal(true); }}>
      ...
      {tab === 'relatorios' && <div className="flex flex-wrap gap-3 rounded-xl border bg-white p-6"><button className="rounded border px-4 py-2 text-sm" onClick={() => downloadCsv('relatorio-completo-projeto.csv', movimentacoesProjeto.map((mov) => ({ data: mov.data, titulo: mov.titulo, projeto_id: mov.projetoId, categoria: mov.categoria, valor_total: mov.valorTotal, status: mov.status })))}>Relatório completo (CSV)</button><button className="inline-flex items-center gap-2 rounded border px-4 py-2 text-sm"><FileText className="h-4 w-4" />PDF (stub)</button></div>}

      <ModalMovimentacao isOpen={openModal} onClose={() => setOpenModal(false)} editData={editingMovement} projects={projects.map((p) => ({ id: p.id, name: p.nome, fundId: p.fundoId }))} funds={funds.map((f) => ({ id: f.id, name: f.nome }))} categories={categories} attachments={attachments} onSubmit={(payload: MovementPayload) => editingMovement ? updateMovement(editingMovement.id, payload) : createMovement({ ...payload, project_id: id, fund_id: payload.fund_id || projeto.fundoId })} onDelete={editingMovement ? async () => { await deleteMovementCascade(editingMovement.id); setOpenModal(false); } : undefined} onUploadAttachment={uploadAttachment} onDeleteAttachment={deleteAttachment} onViewAttachment={viewAttachment} onChanged={refreshAll} />
