...
export function ProjetoDetalhes() {
  ...
  const metrics = useMemo(() => {
    ...
    const entradasPagas = movimentacoesProjeto
      .filter((m) => m.tipo === 'entrada' && m.status === 'pago')
      .reduce((acc, m) => acc + m.valorTotal, 0);

    const saidasPagas = movimentacoesProjeto
      .filter((m) => m.tipo === 'saida' && m.status === 'pago')
      .reduce((acc, m) => acc + m.valorTotal, 0);
    ...
    return {
      saldoDisponivel: Number(projeto?.saldoDisponivel || 0),
      orcado,
      gastoReal: gastoPago,
  ...
  }, [projeto, movimentacoesProjeto, isProjetoMisto]);
    ...
    <div className="space-y-5 p-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-600">
            Tipo: <b>{isProjetoMisto ? 'Misto (sem fundo fixo)' : 'Fundo próprio / com fundo base'}</b>
          </p>
        </div>
              ...
              <table className="w-full">
                <thead className="border-b bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs uppercase">Fundo</th>
                ...
                <tbody>
                  {movimentacoesProjeto.map((mov) => (
                    <tr key={mov.id} className="border-b">
                      ...
                      <td className="px-4 py-3 text-sm">{funds.find((f) => f.id === mov.fundoId)?.nome ?? '—'}</td>
      ...
      {tab === 'relatorios' && (
        <div className="flex flex-wrap gap-3 rounded-xl border bg-white p-6">
          <button
            ...
            onClick={() =>
              downloadCsv(
                'relatorio-completo-projeto.csv',
                movimentacoesProjeto.map((mov) => ({
                  data: mov.data,
                  titulo: mov.titulo,
      ...
      <ModalMovimentacao
        ...
        onSubmit={(payload: MovementPayload) => {
          ...
          const finalPayload: MovementPayload = {
            ...payload,
            project_id: id,
