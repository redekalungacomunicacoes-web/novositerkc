...
export function ProjetoDetalhes() {
  ...
  return (
    <div className="space-y-5 p-8">
      ...
      {tab === 'prestacao' && <div className="space-y-3 rounded-xl border bg-white p-6"><p className="text-sm text-gray-600">Pendentes: {metrics.pendencias}</p><p className="text-sm text-gray-600">Sem documento: {metrics.semDocumento}</p>{movimentacoesProjeto.filter((mov) => mov.status === 'pago').map((mov) => <div className="flex justify-between rounded border p-3 text-sm" key={mov.id}><span>{mov.titulo} • {formatCurrency(mov.valorTotal)}</span><span>{mov.status} • {mov.attachmentsCount || mov.comprovantes.length} comprovante(s)</span></div>)}</div>}
