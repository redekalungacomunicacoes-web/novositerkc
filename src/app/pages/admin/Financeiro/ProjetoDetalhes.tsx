...
export function ProjetoDetalhes() {
  ...
  const metrics = useMemo(() => {
    ...
    return { saldoDisponivel: projeto?.saldoDisponivel || 0, orcado, gastoReal: gastoPago, execucao: orcado > 0 ? (gastoPago / orcado) * 100 : 0, comprovantesPct: movimentacoesProjeto.length > 0 ? (withAttachments / movimentacoesProjeto.length) * 100 : 0, pendencias: movimentacoesProjeto.filter((m) => m.status === 'pendente').length, semDocumento: movimentacoesProjeto.length - withAttachments };
  ...
  const categoriaRows = useMemo(() => {
    ...
    movimentacoesProjeto.filter((m) => m.tipo === 'saida').forEach((m) => map.set(m.categoria || 'Sem categoria', (map.get(m.categoria || 'Sem categoria') ?? 0) + m.valorTotal));
  ...
  return (
    <div className="space-y-5 p-8">
      <div className="flex items-center justify-between"><div><Link to="/admin/financeiro/projetos" className="inline-flex items-center gap-2 text-sm text-[#0f3d2e] hover:underline"><ArrowLeft className="h-4 w-4" />Voltar para projetos</Link><h1 className="mt-2 text-2xl font-semibold">{projeto.nome}</h1></div><button onClick={() => { setEditingMovement(null); setAttachments([]); setOpenModal(true); }} className="inline-flex items-center gap-2 rounded bg-[#0f3d2e] px-4 py-2 text-sm text-white"><Plus className="h-4 w-4" />Nova Movimentação</button></div>
      ...
      {tab === 'prestacao' && <div className="space-y-3 rounded-xl border bg-white p-6"><p className="text-sm text-gray-600">Pendentes: {metrics.pendencias}</p><p className="text-sm text-gray-600">Sem documento: {metrics.semDocumento}</p>{movimentacoesProjeto.map((mov) => <div className="flex justify-between rounded border p-3 text-sm" key={mov.id}><span>{mov.titulo} • {formatCurrency(mov.valorTotal)}</span><span>{mov.status} • {mov.attachmentsCount || mov.comprovantes.length} comprovante(s)</span></div>)}</div>}
      ...
      <ModalMovimentacao isOpen={openModal} onClose={() => setOpenModal(false)} editData={editingMovement} projects={projects.map((p) => ({ id: p.id, name: p.nome, fundId: p.fundoId }))} funds={funds.map((f) => ({ id: f.id, name: f.nome }))} categories={categories} attachments={attachments} onSubmit={(payload: MovementPayload) => editingMovement ? updateMovement(editingMovement.id, payload) : createMovement({ ...payload, project_id: id, fund_id: payload.fund_id || projeto.fundoId })} onDelete={editingMovement ? async () => { await deleteMovementCascade(editingMovement.id); setOpenModal(false); } : undefined} onUploadAttachment={async (file, movementId, payload) => uploadAttachment(file, movementId, payload.fund_id || projeto.fundoId, payload.project_id || id)} onDeleteAttachment={deleteAttachment} onViewAttachment={viewAttachment} onChanged={refreshAll} />
