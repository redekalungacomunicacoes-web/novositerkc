...
export function Fundos() {
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const ok = editingFund ? await updateFund(editingFund.id, form) : await createFund(form);
    if (ok) {
      setOpenModal(false);
      setEditingFund(null);
    }
        ...
        {(funds || []).map((fundo) => (
          <div key={fundo.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3"><Link to={`/admin/financeiro/fundo/${fundo.id}`} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0f3d2e] text-white rounded-lg hover:bg-[#0a2b20] transition-colors text-sm font-medium"><Eye className="w-4 h-4" />Ver Detalhes</Link></div>
              <div className="flex gap-3 mt-3"><button onClick={() => openEdit(fundo)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700"><Edit className="w-4 h-4" />Editar</button><button onClick={() => void deleteFund(fundo.id)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700"><Trash2 className="w-4 h-4" />Excluir</button></div>
            </div>
          </div>
        ))}
      </div>
