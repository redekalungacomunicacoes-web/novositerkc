...
import { formatCurrency, listFunds, type FinanceiroFundo } from './data/financeiro.repo';

export function Fundos() {
  const [fundos, setFundos] = useState<FinanceiroFundo[]>([]);

  useEffect(() => {
    async function load() {
      setFundos(await listFunds());
    }
    void load();
  }, []);

  const totalFundos = useMemo(() => fundos.reduce((acc, f) => acc + (Number(f.totalOrcado) || 0), 0), [fundos]);
  const saldoDisponivel = useMemo(() => fundos.reduce((acc, f) => acc + (Number(f.saldoAtual) || 0), 0), [fundos]);
  const fundosAtivos = useMemo(() => fundos.filter((f) => f.status === 'ativo').length, [fundos]);
  ...
  return (
    <div className="p-8">
      ...
      {fundos.length === 0 && <p className="text-sm text-gray-500 mt-6">Sem dados no período.</p>}
    </div>
  );
}
