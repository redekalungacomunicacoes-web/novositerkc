...
const statusColor = { concluida: "bg-emerald-500", em_andamento: "bg-amber-500", pendente: "bg-sky-500", revisao: "bg-violet-500", cancelada: "bg-slate-500" };

export function Calendar({ onSelectDay }: { onSelectDay: () => void }) {
  const { month, setMonth, setSelectedDate, tasks, view, setView, filters, isLoading } = useCalendarStore();
  ...
  return <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-4 backdrop-blur-xl">
    ...
    <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-7">
      {days.map((day) => {
        ...
        const dayTasks = visibleTasks.filter((t) => t.date <= key && t.endDate >= key);
        return <motion.button whileHover={{ scale: 1.01 }} key={key} onClick={() => { setSelectedDate(key); onSelectDay(); }} className="min-h-32 rounded-2xl border border-white/10 bg-white/[0.03] p-2 text-left hover:border-sky-500/40">
          ...
          <div className="space-y-1">{dayTasks.slice(0, 3).map((t) => <div key={t.id} className="flex items-center gap-1 text-[11px]"><span className={`h-2 w-2 rounded-full ${statusColor[t.status]}`} />{t.title}</div>)}{dayTasks.length > 3 ? <p className="text-[10px] text-slate-400">+{dayTasks.length - 3} tarefas</p> : null}</div>
        </motion.button>;
      })}
    </div>
  {isLoading ? <p className="mt-3 text-xs text-slate-400">Carregando tarefas...</p> : null}</section>;
}
