...
export function MateriaDetalhes() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (resolvedAuthorId) {
        const strictAuthorRes = await supabase
          .from("equipe")
        ...
        const authorData = strictAuthorRes.data || (await supabase
          .from("equipe")
          .select("id, nome, slug, cargo, bio, foto_url, avatar_url, avatar_thumb_path")
          .eq("id", resolvedAuthorId)
          .maybeSingle()).data;
      ...
      if (!authorProfile && found.autor_nome) {
        const byNameRes = await supabase
          .from("equipe")
          .select("id, nome, slug, cargo, bio, foto_url, avatar_url, avatar_thumb_path")
          .ilike("nome", found.autor_nome)
          .limit(1)
          .maybeSingle();

        if (byNameRes.data) {
          authorProfile = {
            id: byNameRes.data.id,
            slug: byNameRes.data.slug || null,
            nome: byNameRes.data.nome || found.autor_nome || "",
            cargo: byNameRes.data.cargo || null,
            bio: byNameRes.data.bio || null,
            foto: getTeamAvatarThumbUrl(byNameRes.data.avatar_thumb_path) || byNameRes.data.avatar_url || byNameRes.data.foto_url || null,
          };
        }
      }
