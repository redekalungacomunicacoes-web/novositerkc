...
export function MateriaDetalhes() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setViewCount(null);
      let found: any = null;
      const extendedSelect = "id, slug, titulo, resumo, capa_url, autor_nome, autor_id, autor_equipe_id, tags, conteudo, content_blocks, hashtags, audio_url, published_at, created_at, status";
      const legacySelect = "id, slug, titulo, resumo, capa_url, autor_nome, autor_id, autor_equipe_id, tags, conteudo, published_at, created_at, status";

      const fetchPublishedMateria = async (field: "slug" | "id", value: string) => {
        ...
        if (res.error && missingColumnError) {
          res = await supabase
            .from("materias")
            .select(legacySelect)
            .eq(field, value)
      ...
      const bySlug = await fetchPublishedMateria("slug", paramValue);

      if (!bySlug.error && bySlug.data && bySlug.data.length > 0) {
        found = bySlug.data[0];
      } else {
        const byId = await fetchPublishedMateria("id", paramValue);
